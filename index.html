...
<head>
  ...
  <title>Coraz贸n p煤rpura 馃挏</title>
  <style>
    :root {
      --background-color: #000000;
      --heart-particle-color-hue-min: 270;
      --heart-particle-color-hue-range: 30;
      --heart-particle-color-lightness: 70%;
      --text-particle-color-hue-min: 280;
...
<body>
  ...
  <script>
    ...
    const mouse = { x: null, y: null, radius: config.mouse.radius * scaleFactor };
    ...
    function drawBackground() {
      ...
      for (let x = 0; x < backgroundCanvas.width; x += 10) {
        for (let y = 0; y < backgroundCanvas.height; y += 10) {
          ...
          bgCtx.fillStyle = `rgba(${red}, 0, ${red}, ${1 - distance * 0.8})`;
    ...
    class Particle {
      constructor(x, y, isTextParticle = false) {
        ...
        this.color = isTextParticle ?
          `hsl(${Math.random() * parseFloat(getComputedStyle(document.documentElement).getPropertyValue('--text-particle-color-hue-range')) + parseFloat(getComputedStyle(document.documentElement).getPropertyValue('--text-particle-color-hue-min'))}, 100%, ${getComputedStyle(document.documentElement).getPropertyValue('--text-particle-color-lightness')})` :
          `hsl(${Math.random() * parseFloat(getComputedStyle(document.documentElement).getPropertyValue('--heart-particle-color-hue-range')) + parseFloat(getComputedStyle(document.documentElement).getPropertyValue('--heart-particle-color-hue-min'))}, 100%, ${getComputedStyle(document.documentElement).getPropertyValue('--heart-particle-color-lightness')})`;
